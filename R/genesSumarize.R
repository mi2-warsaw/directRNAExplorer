...
genesSummarize <- function(bamDataFrame, geneData=directRNAExplorer::TAIR10_genes_tidy, chromosome=NULL, range=0, strand="both", genePart="gene"){
  ...
  if(strand=="neg"){
    positions <- bamDataFrame[bamDataFrame$rname == as.character(chromosome) &
                                      apply(as.data.frame(bamDataFrame$flag), 1, check_neg),
                                    'pos'
                                    ]
    counts <- matchToGene(positions = positions, start=geneData$V4, stop=geneData$V5, geneName=geneData$id, range = range)
    uniqueCounts <- matchToGene(positions = unique(positions),start=geneData$V4, stop=geneData$V5, geneName=geneData$id, range = range)
